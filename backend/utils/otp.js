 ...
 */
async function issueOtp(retailerId, purpose, deliverTo, method) {
  const code = generateOtp();
  const expiresAt = new Date(Date.now() + OTP_TTL_MINUTES * 60 * 1000).toISOString();

  await db.query(
    'INSERT INTO otp_codes (id, retailer_id, code, purpose, expires_at) VALUES ($1, $2, $3, $4, $5)',
    [uuidv4(), retailerId, code, purpose, expiresAt]
  );

  if (method === 'mobile') {
    // TODO: call your SMS provider API here.
...
async function verifyOtp(retailerId, purpose, code) {
  const result = await db.query(
    `SELECT * FROM otp_codes
     WHERE retailer_id = $1 AND purpose = $2 AND consumed = 0
  ...
  await db.query('UPDATE otp_codes SET consumed = 1 WHERE id = $1', [row.id]);
  return { ok: true };
}
